	...
	protected int matchNodes(int firstNode, int lastNode, int idxStart) {
		...
		for (int node = firstNode; node < lastNode;) {
			...
			case OP_RELUCTANTPLUS: {
				...
				// subexpr
				if ((idxNew = matchNodes(next
						+ (short) instruction[next + offsetNext], maxNode, idx)) != -1) {
					return idxNew;
				}
			...
			case OP_ESCAPE:
				...
				case E_BOUND: {
					...
					char cNext = ((search.isEnd(idx)) ? '\n' : search
							.charAt(idx));
					if ((RECharacter.isLetterOrDigit(cLast) == RECharacter
			...
			case OP_POSIXCLASS: {
				...
				{
					boolean isXDigit = ((search.charAt(idx) >= '0' && search
							.charAt(idx) <= '9')
			...
			case OP_ANYOF: {
				...
				for (int i = idxRange; !match && i < idxEnd;) {
					...
					match = ((compareChars(c, s, caseFold) >= 0) && (compareChars(
							c, e, caseFold) <= 0));
				}
