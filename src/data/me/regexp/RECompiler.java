...
public class RECompiler {
	...
	int characterClass() throws RESyntaxException {
		...
		if (idx < len && pattern.charAt(idx) == ':') {
			...
			int idxStart = idx;
			while (idx < len && pattern.charAt(idx) >= 'a'
					&& pattern.charAt(idx) <= 'z') {
				idx++;
			}
		...
		while (idx < len && pattern.charAt(idx) != ']') {
			...
			case '\\': {
				...
				case ESC_CLASS:
					...
					case RE.E_NSPACE:
						range.include(Character.MIN_VALUE, 7, include); // [Min
																		// - \b
																		// )
					...
					case RE.E_NALNUM:
						range.include(Character.MIN_VALUE, '/', include); // [Min
																			// -
																			// '0')
						range.include(':', '@', include); // ('9' - 'A')
						range.include('[', '^', include); // ('Z' - '_')
						range.include('`', include); // ('_' - 'a')
						range.include('{', Character.MAX_VALUE, include); // ('z'
																			// -
																			// Max]
						break;
					...
					case RE.E_NDIGIT:
						range.include(Character.MIN_VALUE, '/', include); // [Min
																			// -
																			// '0')
						range.include(':', Character.MAX_VALUE, include); // ('9'
																			// -
																			// Max]
						break;
		...
		if (greedy) {
			...
			case '{': {
				...
				for (int c = 0; c < bracketMin; c++) {
					// Rewind stream and run it through again - more matchers
					// coming
