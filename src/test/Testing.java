...
public class Testing{
	...
	public static void driver() throws InvalidFieldException{
		...
		// CONFIRMATION/OUTPUT: isCastOff returns true if contestant NOT casted off 
	...
	public static void stub(){
		...
		try {
			...
			System.out.println("TEST 1: ADDING 6(MAX AMOUNT IN THIS CASE) CONTESTANTS");
			if(response!=0)
				System.out.println("\tFAILED ADDING CONTESTANTS UP TO MAX AMOUNT");
			else
				System.out.println("\tSUCCESFULLY ADDED CONTESTANTS UP TO MAX AMOUNT");
			...
			if(response==-2)
				System.out.println("\tPASSED: COULD NOT ADD MORE THAN MAX CONTESTANTS");
			else
				System.out.println("\tFAILED: COULD ADD MORE THAN MAX CONTESTANTS");
			
			
			System.out.println("\n\nREMOVING A CONTESTANT TO TEST FOR SAME ID");
			list.remove(5);
			System.out.println("TEST 3: ADDING CONTESTANT WITH ID i5(which already exists)");
			Contestant c8 = new Contestant();
			c8.setID("i5");
			response = addContestant(c8);
			if(response==-1)
				System.out.println("\tPASSED: COULD NOT ADD CONTESTANT WITH SAME ID");
			else
				System.out.println("\tFAILED: COULD ADD CONTESTANT WITH SAME ID");
	...
	//STUB to add contestants RETURNS -1 IF ID INVALID, -2 IF MAX CONTESTANT REACHED
